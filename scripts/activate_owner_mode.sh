...
# ENTER TO DATASET DVC DIRECTORY
cd datasets/D-$DATASET_NAME
echo "Entered to $DATASET_NAME directory (CURRENT PATH: $(pwd))"
...
dvc remote modify storage gdrive_service_account_json_file_path "$owner_auth_path"
...
dvc remote modify uploads gdrive_service_account_json_file_path "$owner_auth_path"
